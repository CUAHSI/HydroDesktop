...
Namespace Controls
    Public Class DeriveNewDataSeries
        ...
        Private ReadOnly _cEditView As EditView
        ...
        Public Sub New(ByVal seriesId As Int32, ByRef cEditView As EditView, ByRef seriesSelector As ISeriesSelector)
...
#Region "QualityControlLevel data accesses"
        ...
        Private Sub btnQualityControlLevel_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnQualityControlLevel.Click
            ShowQualityControlLevelTableManagement()
        End Sub
        ...
        Private Sub SetDefaultQualityControlLevel()
            Dim currentQualityControlLevelID = dataSeriesRepository.GetQualityControlLevelID(_SelectedSeriesID.ToString)
            ddlQualityControlLevel.SelectedValue = currentQualityControlLevelID
        End Sub

        Private Sub ShowQualityControlLevelTableManagement()
            Dim frm = New QualityControlLevelTableManagement()
            frm.initialize(If(IsDBNull(ddlQualityControlLevel.SelectedValue), CType(Nothing, Long?), Convert.ToInt64(ddlQualityControlLevel.SelectedValue)))
            If frm.ShowDialog() = DialogResult.OK Then
                FillQualityControlLevel()
                ddlQualityControlLevel.SelectedValue = frm.QualityControlLevelID
            End If
        End Sub
...
#Region "Methods data accesses"
        ...
        Private Sub btnMethods_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnMethods.Click
            ShowMethodTableManagement()
        End Sub

        Private Sub ddlMethods_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As EventArgs) Handles ddlMethods.SelectedIndexChanged
            If ddlMethods.SelectedIndex = ddlMethods.Items.Count - 1 Then
                ShowMethodTableManagement()
            End If
        End Sub

        Private Sub ShowMethodTableManagement()
            Dim frm = New MethodTableManagement()
            frm.initialize(If(IsDBNull(ddlMethods.SelectedValue), CType(Nothing, Long?), Convert.ToInt64(ddlMethods.SelectedValue)))
            If frm.ShowDialog() = DialogResult.OK Then
                FillMethods()
                ddlMethods.SelectedValue = frm.MethodID
            End If
        End Sub

        Private Sub SetDefaultMethods()
            Dim repo = RepositoryFactory.Instance.Get(Of IMethodsRepository)()
            Dim derivedMethod = repo.GetMethodID(DERIVED_METHOD_DESCRIPTION)
...
#Region "Variables data accesses"
        ...
        Private Sub ddlVariable_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As EventArgs) Handles ddlVariable.SelectedIndexChanged
            If ddlVariable.SelectedIndex = ddlVariable.Items.Count - 1 Then
                ShowVariablesTableManagment()
            End If
        End Sub

        Private Sub btnVariable_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles btnVariable.Click
            ShowVariablesTableManagment()
        End Sub

        Private Sub ShowVariablesTableManagment()
            Dim frm = New VariablesTableManagement()
            frm.initialize(If(IsDBNull(ddlVariable.SelectedValue), CType(Nothing, Long?), Convert.ToInt64(ddlVariable.SelectedValue)))
            If frm.ShowDialog() = DialogResult.OK Then
                FillVariable()
                ddlVariable.SelectedValue = frm.VariableID
            End If
        End Sub

        Private Sub SetDefaultVariable()
            Dim currentVariableID = _derivedVariable.Id
            ddlVariable.SelectedValue = currentVariableID
        End Sub
