...
namespace Search3
{
    public class SearchPlugin : Extension, ISearchPlugin
    {
        ...
        private void AddSearchRibbon()
        {
            ...
            head.Add(new RootItem(_shaleKey, Msg.Shale) { SortOrder = -5 });
            ...
            SimpleActionItem dummy = new SimpleActionItem(_shaleKey, "Dummy", rbDrawBox_Click) { GroupCaption = Msg.Area, ToggleGroupKey = Msg.Area, Visible = false };
            head.Add(dummy);
            ...
            SimpleActionItem dummy2 = new SimpleActionItem(_shaleKey, "Dummy", rbDrawBox_Click) { GroupCaption = Msg.Controls, ToggleGroupKey = Msg.Controls, Visible = false };
            head.Add(dummy2);


            head.Add(rbDrawBox = new SimpleActionItem(_shaleKey, Msg.Draw_Rectangle, rbDrawBox_Click) { LargeImage = ShaleDataNetwork.Properties.Resources.Draw_Box_32, SmallImage = ShaleDataNetwork.Properties.Resources.Draw_Box_16, GroupCaption = Msg.Area, ToggleGroupKey = Msg.Area });
            _searchSettings.AreaSettings.AreaRectangleChanged += Instance_AreaRectangleChanged;

            head.Add(new SimpleActionItem(_shaleKey, Msg.Select_By_Attribute, rbAttribute_Click) { GroupCaption = Msg.Area, SmallImage = ShaleDataNetwork.Properties.Resources.select_table_16 });

            head.Add(rbSelect = new SimpleActionItem(_shaleKey, Msg.Select_Features, rbSelect_Click) { ToolTipText = Msg.Select_Features_Tooltip, SmallImage = ShaleDataNetwork.Properties.Resources.select_poly_16, GroupCaption = Msg.Area, ToggleGroupKey = Msg.Area, });
            _searchSettings.AreaSettings.PolygonsChanged += AreaSettings_PolygonsChanged;

            head.Add(new SimpleActionItem(_shaleKey, Msg.Deselect_All, delegate { IEnvelope env; App.Map.MapFrame.ClearSelection(out env); }) { GroupCaption = Msg.Area, ToolTipText = Msg.Deselect_All_Tooltip, SmallImage = ShaleDataNetwork.Properties.Resources.deselect_16x16 });
            ...
            head.Add(_currentView = new SimpleActionItem(_shaleKey, Msg.Current_View, CurrentView_Click) { GroupCaption = Msg.Area, ToggleGroupKey = Msg.Area, ToolTipText = Msg.Current_View_Tooltip, LargeImage = ShaleDataNetwork.Properties.Resources.current_view_32, SmallImage = ShaleDataNetwork.Properties.Resources.current_view_16 });
            ...
            rbEndDate = new TextEntryActionItem { Caption = Msg.TimeRange_End, GroupCaption = Msg.Time_Range, RootKey = _shaleKey, Width = 70 };
            head.Add(rbEndDate);
            rbEndDate.PropertyChanged += rbEndDate_PropertyChanged;
            UpdateDatesCaption();

            head.Add(new SimpleActionItem(_shaleKey, Msg.Select_Dates, rbDate_Click) { GroupCaption = Msg.Time_Range, LargeImage = ShaleDataNetwork.Properties.Resources.select_date_v1_32, SmallImage = ShaleDataNetwork.Properties.Resources.select_date_v1_16 });
        ...
        {
            if (e.SelectedRootKey == _shaleKey)
            {
        ...
        private IEnumerable<IMapPointLayer> ShowSearchResults(SearchResult searchResult)
        {
            ...
            foreach (var key in searchResult.ResultItems)
            {
                ...
                Debug.WriteLine("The other part: " + string.Format(ShaleDataNetwork.Properties.Settings.Default.SearchResultNameMask, key.ServiceCode));
                var fs = key.FeatureSet;
                var filename = Path.Combine(hdProjectPath,
                                            string.Format(ShaleDataNetwork.Properties.Settings.Default.SearchResultNameMask, key.ServiceCode));
        ...
        private void RecreateKeywordGroup()
        {
            ...
            if (_currentKeywords != null)
            {
                ...
                dummy = new SimpleActionItem(_shaleKey, "Dummy", null) { GroupCaption = Msg.Keyword };
            ...
            if (dummy != null)
            {
                ...
                App.HeaderControl.SelectRoot(_shaleKey);
            }
        ...
        private void ChangeWebServicesIcon(WebServiceNode webServiceNode = null)
        {
            ...
            {
                rbServices.LargeImage = ShaleDataNetwork.Properties.Resources.server_32;
                rbServices.SmallImage = ShaleDataNetwork.Properties.Resources.server_16;
                return;
            }
