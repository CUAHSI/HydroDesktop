...
namespace FacetedSearch
{
    ...
    public partial class SearchFacetSpecifier : UserControl
    {
        ...
        public List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> PreviousSelectedOntologyElements;      // serialized ontology elements that have been specified by previous SearchFacetSpecifiers
        public List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> MyFullFacetSet;                        // The entire catalog of ontological terms, unfiltered
        public List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> RemainingFacets;                       // response from XML webservice. These are the facets that are remaining given the user's selections.        
        public List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> Orphans;
        public List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> MySelectedFacets;
        ...
        private void ExposeNode(TreeNode p, int KeyAsInt, string NameMatch)
        {
            ...
            {
                if ((p.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First().cConceptName != NameMatch)
                {
                    p.Text = string.Format("{0} ({1})", (p.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First().cConceptName, NameMatch);                    
                }
        ...
        private void RecursiveFontSetter(TreeNode p, FontStyle f, Boolean ResetText)
        {
            ...
            if (ResetText)
            {
                ...
                {
                    p.Text = (p.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First().cConceptName;
                }
        ...
        public SearchFacetSpecifier(FacetedSearchControl ParentForm, List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> FullFacetSet, List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> SelectedThusFar, List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> RemainingFacetsIn, int MyIndex)
        {
            ...
            MySelectedFacets = new List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>();
        ...
        public SearchFacetSpecifier(List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> RemainingFacetsIn, int MyIndex)
        {
            InitializeComponent();
            RemainingFacets = RemainingFacetsIn;
            MyFacetIndex = MyIndex;
        ...
        {                        
            ...
            List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> FailedListings = new List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>();
            List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> AlreadyListed = new List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>();
            
            // insert roots if they are recorded as having data attached to them
            foreach (List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> os in RemainingFacets.Where(r => r.IsRoot == true).GroupBy(r => r.cConceptID).ToList())
            {
            ...
            Stack<List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>> AddNodes = new Stack<List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>>();
            List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> NowElements = new List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>();
            Boolean FoundRoot;
            foreach (IGrouping<int, FacetedSearch.CUAHSIFacetedSearch.OntologyElement> g in RemainingFacets.GroupBy(r => r.cConceptID))
            {
                AddNodes.Clear();
                NowElements.Clear();                              
                ...
                {
                    List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> ThisNode = AddNodes.Pop();
                    try
                    {
            ...
            {
                string FailedAdds = "";            
                foreach (FacetedSearch.CUAHSIFacetedSearch.OntologyElement o in FailedListings)
                {
        ...
        private void AddChildrenToFacetSearchRoot(TreeNode root)
        {
            ...
            foreach (List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> os in Orphans
                .Where(r => r.cConceptID == (root.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First().cConceptID)
        ...
        /// <returns></returns>
        private static TreeNode CreateCUAHSITreeNode(List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> os)
        {            
        ...
        /// </summary>
        /// <returns></returns>
        private List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> CollectTotalSelectedFacets()
        {
            ...
            foreach (TreeNode n in FacetTree.Nodes)
            {
                ...
                {
                    MySelectedFacets.Add((d.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First());
                }
            }

            List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement> TotalSelectedFacets = new List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>();
        ...
        {
            TreeNode NowNode = n;
            while ((NowNode.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First().IsRoot == false)
            {
                NowNode = n.Parent;
            }
        ...
        private Dictionary<int, string> GetExposeChildren(TreeNode p, String src, Dictionary<int, string> exp)
        {
            foreach (TreeNode n in p.Nodes)
            {
                ...
                {
                    exp.Add((n.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First().cConceptID, null); // null signifies that 
                }                
                ...
                {*/
                    ...
                    List<String> TagNames = (n.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).Select(r => r.cConceptName).ToList();
                    TagNames.AddRange((n.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).Select(r => r.Synonym));
                    ...
                    {
                        exp.Add((n.Tag as List<FacetedSearch.CUAHSIFacetedSearch.OntologyElement>).First().cConceptID, TagNames.Where(r => r.IndexOf(src, StringComparison.OrdinalIgnoreCase) >= 0).First());
                    }
