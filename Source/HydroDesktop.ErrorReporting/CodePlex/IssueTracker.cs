...
namespace HydroDesktop.ErrorReporting.CodePlex
{
    internal class IssueTracker
    {
        ...
        public void CreateIssue(Issue issue)
        {
            ...
            var fileName = issue.FileToAttach;
            byte[] fileData = null;
            if (!String.IsNullOrEmpty(fileName) && File.Exists(fileName))
            {
                var fi = new FileInfo(fileName);
                if (fi.Length <= 4*1024*1024) // Max 4 MB
                {
            ...
            if (fileData != null)
            {
                form.Elements.Add(new HtmlElement("PostedFile_text", @"C:\fakepath\" + Path.GetFileName(fileName)));
            ...
            _browser.Post(createIssueUrl, form, createIssueUrl, true);
        }
    }
}
